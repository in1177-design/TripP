...
const CAT_META: Record<string, CatMeta> = {
  ...
  'אוכל':     { icon: '🍽️', color: '#2dc76d' }, // legacy alias
  'אחר':      { icon: '💳', color: '#95a5a6' },
};
function getCat(cat: string): CatMeta { return CAT_META[cat] ?? CAT_META['אחר']; }

// Categories displayed in the picker (no legacy duplicates)
  ...
  return (
    <svg viewBox="0 0 130 130" className="exp-donut-svg">
      {slices.map((sl, i) => {
        ...
        const len  = frac * C - 1.8;   // 1.8px gap between slices
        const dashOff = -off;
        off += frac * C;
        return (
...
export default function ExpensesTab({ trip, onChange }: Props) {
  ...
  // ── Category breakdown (for primary currency)
  ...
  const grouped = useMemo(() => {
    ...
    return Object.entries(map)
      ...
      .map(([date, exps]) => {
        ...
        exps.forEach(e => { dayTot[e.currency] = (dayTot[e.currency] || 0) + e.amount; });
  ...
  function save() {
    ...
    onChange({ ...trip, expenses: [...expenses, { ...form, id: generateId() }] });
    setShowModal(false);
  }
  function remove(id: string) {
    onChange({ ...trip, expenses: expenses.filter(e => e.id !== id) });
  }
  function openModal() { setForm(blankForm()); setShowModal(true); }
  ...
  return (
    <div className="exp-root">
      ...
      {/* ── CURRENCY FILTER (multi-currency trips) ── */}
            ...
            <div key={g.date} className="exp-group">
              <div className="exp-group-hdr">
                ...
                <span className="exp-group-tot">
                  {Object.entries(g.dayTot).map(([cur, amt]) => `${amt.toFixed(2)} ${cur}`).join(' · ')}
                </span>
              </div>
              {g.exps.map(exp => {
                ...
                return (
                  <div key={exp.id} className="exp-row">
                    ...
                    <div className="exp-row-end">
                      <span className="exp-row-amt">{exp.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
