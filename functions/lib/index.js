...
function guessCountry(destination) {
    ...
    if (/poland|פולין|krak[oó]w|warsaw|warszawa|zakopane|gdańsk|gdansk|wrocław|wroclaw|wieliczka|auschwitz|o[sś]wi[eę]cim|zator|energylandia|tatry|tatra|chocho|bania|białka|bialka/.test(d))
        return 'PL';
    if (/france|צרפת|paris|פריז|lyon|nice|marseille/.test(d))
        return 'FR';
    if (/italy|italia|איטליה|rome|roma|רומא|milan|florence|venice|firenze/.test(d))
        return 'IT';
    if (/spain|españa|ספרד|barcelona|ברצלונה|madrid|מדריד|seville|sevilla/.test(d))
        return 'ES';
    if (/netherlands|holland|הולנד|amsterdam|אמסטרדם/.test(d))
        return 'NL';
    if (/greece|grecia|יוון|athens|athina|אתונה/.test(d))
        return 'GR';
    if (/israel|ישראל|tel.?aviv|תל.?אביב|jerusalem|ירושלים|haifa|חיפה/.test(d))
        return 'IL';
    return null;
}
function mockSearch(query, countryCode) {
    ...
    for (const p of MOCK_PLACES) {
        ...
        if (countryCode && p.country !== countryCode)
            continue;
        let score = 0;
        const nameLow = p.name.toLowerCase();
        const nameHeLow = (p.nameHe || '').toLowerCase();
        const descLow = (p.description || '').toLowerCase();
        if (nameLow === q || nameHeLow === q)
            score += 100; // exact match
        else if (nameLow.startsWith(q) || nameHeLow.startsWith(q))
            score += 60;
        else if (nameLow.includes(q) || nameHeLow.includes(q))
            score += 40;
        else if (p.address.toLowerCase().includes(q))
            score += 20;
        else if (p.category.includes(q))
            score += 10;
...
exports.searchPlaces = (0, https_1.onCall)({ maxInstances: 10, timeoutSeconds: 10, cors: PLACES_CORS }, async (request) => {
    ...
    const countryCode = guessCountry(trip.destination || '');
